...
def keyword_to_titles(metadata):
    ...
    for article in metadata:
        if len(article) > 0:
            for item in article[4]:
                if item not in res:
                    res[item] = [article[0]]
                else:
                    (res[item]).append(article[0])
    return res
...
def title_to_info(metadata):
    ...
    for article in metadata:
        if len(article) > 0:
            title = article[0]
            res[title] = {}
            res[title]['author'] = article[1]
            res[title]['timestamp'] = article[2]
            res[title]['length'] = article[3]
    return res
...
def key_by_author(article_titles, title_to_info):
    ...
    for article in article_titles:
        name = title_to_info[article]['author']
        if name not in res:
            res[name] = [article]
        else:
            (res[name]).append(article)
    return res
...
def articles_from_year(year, article_titles, title_to_info):
    ...
    for article in article_titles:
        if title_to_info[article]['timestamp'] >= start and title_to_info[article]['timestamp'] <= end:
            res.append(article)
    return res
